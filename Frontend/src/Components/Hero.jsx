      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 pt-32 pb-16">
        <div className="flex flex-col items-center justify-center min-h-[85vh] text-center space-y-6">
          ...
          <div className="space-y-6 max-w-4xl">
            <h1 className={`text-6xl lg:text-7xl font-bold leading-tight transition-all duration-700 delay-100 ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            }`}>
          ...
          {/* Description */}
          <p className={`text-lg lg:text-xl text-gray-600 leading-relaxed max-w-3xl transition-all duration-700 delay-200 ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          }`}>
          ...
          <div className={`flex flex-col sm:flex-row gap-4 transition-all duration-700 delay-300 ${
            ...
            <button className="group relative bg-gradient-to-r from-orange-500 to-orange-600 text-white px-8 py-2 rounded-full font-semibold hover:from-orange-600 hover:to-orange-700 transition-all duration-300 flex items-center justify-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 overflow-hidden">
              {/* Shine effect */}
              <div onClick={()=>setIsModalOpen(true)} className="cursor-pointer absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
              <span>Start Your Project</span>
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform duration-300" />
            </button>

            <button className="group relative border-2 border-gray-300 text-gray-700 px-8 py-3 rounded-full font-semibold hover:border-gray-800 hover:text-gray-900 hover:bg-gray-50 transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-sm hover:shadow-md">
              <span>View Our Work</span>
            </button>
          </div>

          {/* Services Grid */}
          <div className={`grid grid-cols-2 lg:grid-cols-5 gap-4 pt-4 w-full max-w-4xl transition-all duration-700 delay-400 ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          }`}>
            {[
              'Web Dev',
